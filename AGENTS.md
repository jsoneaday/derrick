...
9. Host never implements vendor APIs. The factory guest (Go built from vendor docs) owns connect, send, and receive — including HTTP shape, display names, and filtering. Host forwards `http.request` as declared, attaches secrets, and stores `result.emit`. Do not add Slack/Telegram/Discord/WhatsApp HTTP clients, form encoders, or user lookups in Structure, HostUI, or DerrickBackend. `contracts/vendors/*.json` and MCP Slack harnesses are guest/builder assets, not host runtime. Warn if a request would put vendor protocol code in the host.
