...
public enum MCPToolOutcomeSemantics: Sendable {
    public static func isError(toolName: String, text: String, transportIsError: Bool) -> Bool {
        ...
        switch toolName {
        case "python_script_exec":
            return PythonScriptExecutionResult.indicatesFailure(inJSON: text)
        default:
            return false
        }
    }
    ...
    public static func errorMessage(toolName: String, text: String) -> String? {
        switch toolName {
        case "python_script_exec":
            return PythonScriptExecutionResult.failureSummary(inJSON: text)
        default:
            return nil
        }
    }
...
extension PythonScriptExecutionResult {
    ...
    public static func decode(fromJSON text: String) -> PythonScriptExecutionResult? {
        guard let data = text.data(using: .utf8) else { return nil }
        return try? JSONDecoder().decode(PythonScriptExecutionResult.self, from: data)
    }
