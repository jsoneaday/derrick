-- policy_approvals.application_name referenced policy_rules.application_name,
-- which is not UNIQUE/PK. SQLite reports "foreign key mismatch" on every INSERT.
-- Approvals also use synthetic rule_id values (runtime-confirmation), so they
-- cannot reference policy_rules(id) either. Drop the invalid FK; keep the data.
