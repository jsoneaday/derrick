...
- **4 concurrent turns** keeps LLM parallelism reasonable on one Mac. It is **independent** of Docker: guest scripts max 1, crawler max 2, file extractor max 1 (separate queues, recreate-on-handoff, images cached). Parallel `script_exec` workers queue even when turn concurrency allows 4.
